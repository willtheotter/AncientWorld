  ...
  return (
    <div className="w-full h-[70vh] bg-gradient-to-b from-amber-900/20 to-egyptian-blue/20 rounded-lg flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mt-2">Exploring 72 legendary sites across 8 civilizations</p>
      </div>
    </div>
  )
}

// Civilization stats for the homepage
const civilizationStats = [
  { name: 'Egypt', icon: '𓋴', count: 30, color: 'from-amber-600 to-orange-600' },
  { name: 'Nubia', icon: '𓋹', count: 6, color: 'from-emerald-600 to-teal-600' },
  { name: 'Punt', icon: '𓆣', count: 6, color: 'from-blue-600 to-cyan-600' },
  { name: 'Phoenicia', icon: '𓂀', count: 6, color: 'from-purple-600 to-pink-600' },
  { name: 'Babylon', icon: '𓊨', count: 6, color: 'from-yellow-600 to-amber-600' },
  { name: 'Hebrew', icon: '𓅃', count: 6, color: 'from-indigo-600 to-blue-600' },
  { name: 'Hittite', icon: '𓁟', count: 6, color: 'from-red-600 to-orange-600' },
  { name: 'Greece', icon: '𓇳', count: 6, color: 'from-cyan-600 to-blue-600' },
]
    ...
    <div className="space-y-10">
      ...
      >
        ...
        <p className="text-xl md:text-2xl text-gray-100 max-w-3xl mx-auto drop-shadow-md">
          Journey through 72 legendary sites across Egypt, Nubia, Phoenicia, Babylon, Greece, and beyond
        </p>
        <div className="flex gap-3 justify-center text-2xl text-gold">
      ...
      >
        {civilizationStats.map((civ, index) => (
          ...
          >
            ...
            <div className="text-xs font-semibold text-white">{civ.name}</div>
      ...
      >
        {[
          { label: 'Civilizations', value: '8', icon: '🏛️' },
          { label: 'Historical Sites', value: '72+', icon: '🗺️' },
          { label: 'Years of History', value: '5,000+', icon: '📅' },
          { label: 'Interactive Maps', value: '1', icon: '🌍' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, scale: 0.9 }}
